...
from src.ui.theme import apply_theme, render_microcopy, render_section, render_source_quote
        ...
        with st.expander(
            ...
            if chk:
                ...
                for i, piece in enumerate(chk[:5], 1):
                    ...
                    render_source_quote(f"{preview}{'...' if truncated else ''}")
            if ent_cnt:
                st.markdown("**命中实体**")
                ent_lines: list[str] = []
...
def render_chat_tab() -> None:
    ...
    render_section(
        "智能问答",
        "基于意图识别选择检索策略，回答后保留参考文献、片段、实体与关系线索。",
        "Query",
    )
